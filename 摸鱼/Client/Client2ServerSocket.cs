...
namespace 摸鱼.Client
{
    public class Client2ServerSocket : Singleton<Client2ServerSocket>
    {
        //客户端socket
        ...
        private void ReceiveCallBack(IAsyncResult ar)
        {
            try
            {
                ...
                {
                    //服务端" + m_Socket.RemoteEndPoint + "断开链接
                    try
                    {

                        //服务端发生未处理的错误"
                        Global.Main.ServerDisConnection();
                    }
                    catch { }
                }
            }
                    ...
                    if (m_ReceiveQueue.Count > 0)
                    {
                        ...
                        if (newCrc == crc)
                        {
                            ...
                            using (ZyueMemoryStream ms = new ZyueMemoryStream(bufferNew))
                            {
                                ...
                                //交给观察者分配处理
                                ClientEventObserver.Instance.Dispatch(protoCode, protoContent, this);
